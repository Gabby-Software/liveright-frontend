...
import Styles from './add-invoice-modal-1.styles';
                    ...
                    <Form>
                        ...
                        <FormRow>
                            <FormSelect name={'service_type'} label={t('invoices:service-type.title')}
                                        options={serviceTypeOptions} onUpdate={val => update('service_type', val)}/>
                            {
                                form.values.service_type === t(`invoices:service-type.${serviceTypes.PT_SESSION}`) ? (
                                    <FormInputLabeled name={'session_expired'} onUpdate={update}
                                                      label={t('invoices:session-expired')}/>
                                ) : form.values.service_type === t(`invoices:service-type.${serviceTypes.OTHER}`) ? (
                                    <FormInputLabeled name={'other'} label={t('invoices:other')} onUpdate={update}/>
                                ) : null
                            }
                        </FormRow>
