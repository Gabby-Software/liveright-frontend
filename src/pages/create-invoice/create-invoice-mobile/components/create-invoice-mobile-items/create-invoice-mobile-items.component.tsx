    ...
    return (
        <Styles>
                            ...
                            ({values}: FormikProps<InvoiceItemType>) => (
                                <Form>
                                    ...
                                    <FormButton htmlType={'submit'}
                                                type={'default'}>{active===-1?'Add to invoice':'Update item'}</FormButton>
                                </Form>
                            )
                        }
                    </Formik>
            ...
            <div className={'ci-items__summary'}>
                <SummaryItem label={t('invoices:subtotal')} value={asMoney(asPrice(items.reduce((a, b) => a+(+b.unit_price)*b.quantity,0)))}/>
                <SummaryItem label={t('invoices:discount')} value={asMoney(asPrice(items.reduce((a, b) => a+(+b.unit_price)*b.quantity*b.discount_percent/100,0)))}/>
                <SummaryItem label={t('invoices:vat')} value={asMoney(asPrice(items.reduce((a, b) => a+(+b.unit_price)*b.quantity*(1-b.discount_percent/100)*b.tax_rate/100,0)))}/>
                <Hr/>
                <SummaryItem label={t('invoices:total')} value={asMoney(asPrice(items.reduce((a, b) => a+(+b.unit_price)*b.quantity*(1-b.discount_percent/100)*(1+b.tax_rate/100),0)))}/>
            </div>
