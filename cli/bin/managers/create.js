...
const camelCase = name => name.split('-').map(s => s[0].toUpperCase() + s.substring(1).toLowerCase()).join('');
...
`;
const componentSCSSTemplate = (extraDir) => `import styled from "styled-components";
...
export default ${camelCase(name)};
`;
const managerTemplate = name => `export default class ${camelCase(name)}Manager {
...
const create = ({info, name, n, dir, d}) => {
    if (!createTypes[info[0]]) return logger.error(`Invalid option. available options ${Object.keys(createTypes).join(', ')}`);
